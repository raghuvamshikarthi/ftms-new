...
public final class ApplicationThread {
    ...
    public static void start() {
        ...
        db = new Handler(dbThread.getLooper());
            ...
            public static void bgndRemove(@Nullable final Future<?> runner, final boolean force) {
                if (null != runner) runner.cancel(force);
            }


            public final static Future<?> bgndPost(final String clazz, final String msg, final int priority, @NonNull final Runnable runable) {
            ...
            @Nullable
            public static Future<?> bgndPost(final String clazz, final String msg, @NonNull final Runnable run) {
                return bgndPost(clazz, msg, 10, run); }

            @NonNull
            public static Runnable nuiPost(final String clazz, @Nullable final String msg, @NonNull final Runnable runable, final long delay) {
            ...
            @NonNull
            public static Runnable dbPost(final String clazz, @Nullable final String msg, @NonNull final Runnable run, final long delay) {
                final Runnable runner = new Runnable() {
                    public void run() {
                        final long startTime = System.nanoTime();
                        try {
            ...
            @NonNull
            public static Runnable uiPost(final String clazz, @Nullable final String msg, @NonNull final Runnable run, final long delay) {
                final Runnable runner = new Runnable() {
                    public void run() {
                        final long startTime = System.nanoTime();
                        try {
            ...
            @NonNull
            public static Runnable nuiPost(final String clazz, final String msg, @NonNull final Runnable run) { return nuiPost(clazz, msg, run, THREAD_DELAY); }

            @NonNull
            public static Runnable dbPost(final String clazz, final String msg, @NonNull final Runnable run) { return dbPost(clazz, msg, run, THREAD_DELAY); }

            @NonNull
            public static Runnable uiPost(final String clazz, final String msg, @NonNull final Runnable run) { return uiPost(clazz, msg, run, THREAD_DELAY); }
    ...
    public static abstract class OnComplete<T> implements Runnable {
        ...
        public void run() {
            Log.INSTANCE.e(LOG_TAG, "run not implemented!");
        }
        ...
        private void execute() {
            ...
            switch (mode) {
                default:
                    ...
                    Log.INSTANCE.e(LOG_TAG, LOG_TAG, new RuntimeException("UNRECOGNIZED OnComplete mode " + mode + " posting to NUI"));
    ...
    public static abstract class Observable {
        ...
        public final void add(@Nullable final Object observer) {
            synchronized (observers) {
                if (null == observer) throw new IllegalArgumentException("observer is null");
                if (observers.contains(observer)) {

                    Log.INSTANCE.e(LOG_TAG, "ERROR already contains class:" + LOG_TAG + Log.INSTANCE.getStackTraceString(new Throwable()));
                    return;
                }
        ...
        public final void remove(final Object observer) {
            synchronized (observers) {
                if (!observers.contains(observer)) {
                    ...
                    Log.INSTANCE.e(LOG_TAG, "ERROR does not contain class:" + LOG_TAG + Log.INSTANCE.getStackTraceString(new Throwable()));
                    return;
                }
        ...
        public static abstract class Message {
            ...
            public void event(final int id, final Object obj, final Object... param) {
                Log.INSTANCE.v(LOG_TAG, "event(int,object,object) not implemented");
            }
        }
    }
